...
const Book1= () => {
  ...
  const getPdf = async () => {
    try {
      const result = await axios.get("http://localhost:5000/get-files10");
  ...
  const submitImage = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/upload-10", formData, {
  ...
  const deletePdf = async (pdfId) => {
    try {
      await axios.delete(`http://localhost:5000/delete10/${pdfId}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="bg-white shadow-md p-6 rounded-lg mb-6">
        ...
        <p className="text-gray-600 mt-2">Hi, welcome to the Journals section!</p>
      </div>
