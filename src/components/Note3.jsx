...
const Note3 = () => {
  ...
  const getPdf = async () => {
    try {
      const result = await axios.get("http://localhost:5000/get-files7");
  ...
  const submitImage = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/upload-7", formData, {
  ...
  const deletePdf = async (pdfId) => {
    try {
      await axios.delete(`http://localhost:5000/delete7/${pdfId}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="bg-white shadow-md p-6 rounded-lg mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Third Year Personalized Notes</h1>
        <p className="text-gray-600 mt-2">Hi, welcome to the Third Year Personalized Notes section!</p>
      </div>
