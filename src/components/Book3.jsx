...
const Book3 = () => {
  ...
  const getPdf = async () => {
    try {
      const result = await axios.get("http://localhost:5000/get-files3");
  ...
  const submitImage = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/upload-3", formData, {
  ...
  const deletePdf = async (pdfId) => {
    try {
      await axios.delete(`http://localhost:5000/delete3/${pdfId}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="bg-white shadow-md p-6 rounded-lg mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Third Year Reference Books</h1>
        <p className="text-gray-600 mt-2">Hi, welcome to the Third Year Reference Books section!</p>
      </div>
