...
const Note1 = () => {
  ...
  const submitImage = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/upload-6", formData, {
  ...
  const deletePdf = async (pdfId) => {
    try {
      await axios.delete(`http://localhost:5000/delete6/${pdfId}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="bg-white shadow-md p-6 rounded-lg mb-6">
        <h1 className="text-3xl font-bold text-gray-800">First Year Personalized Notes</h1>
        <p className="text-gray-600 mt-2">Hi, welcome to the First Year Personalized Notes section!</p>
      </div>
